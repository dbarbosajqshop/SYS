...
export default class AuditLogTaxController {
  static async getAllLogs(req, res, next) {
    ...
    try {
      const logs = await AuditLogTax.find(filter)
        .sort({ timestamp: -1 })
        .skip((page - 1) * limit)
        .limit(limit)
        .populate("userId", "name email");

      const total = await AuditLogTax.countDocuments(filter);
      ...
      console.error("Erro ao buscar logs de taxa:", error);
      return next(error);
    }
  }
}